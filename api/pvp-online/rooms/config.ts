...
import { getUserRoomMembership, updateRoomConfig } from '../../_lib/pvpOnlineRooms.js';
...
async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const result = await updateRoomConfig(userId, roomId, config, roomVersion);
  if (!result.ok) {
...
export default withProductionControl('api/pvp-online/rooms/config.ts', handler);
