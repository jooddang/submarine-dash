...
import { getUserRoomMembership, setReadyState } from '../../_lib/pvpOnlineRooms.js';
...
async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const result = await setReadyState(userId, roomId, ready, roomVersion);
  if (!result.ok) {
...
export default withProductionControl('api/pvp-online/rooms/ready.ts', handler);
