...
import { getUserRoomMembership, cancelRoom } from '../../_lib/pvpOnlineRooms.js';
...
async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const result = await cancelRoom(userId, roomId, roomVersion);
  if (!result.ok) {
...
export default withProductionControl('api/pvp-online/rooms/cancel.ts', handler);
