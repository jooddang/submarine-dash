// Pure deterministic simulation — no audio, no canvas, no browser APIs.
// Side effects are collected into the `effects` array parameter.

import type { Platform } from "../types";
import type { PvpPlayerState, PvpPowerUpMode, PvpRescueState, PvpTubeRescueState, ScorePopup } from "../pvp/pvpTypes";
import type { SimulationSideEffect } from "./events";
import type { SeededRNG } from "./rng";
import * as Constants from "../constants";
import { generateNextSegment, createBubbleSeeded, spawnBackgroundEntitySeeded } from "./world";
...
export function attemptJump(
  ...
  if (s.isSwordfishActive) {
    ...
    player.boostTimer = 0;
    effects.push({ type: "PLAY_SOUND", sound: "jump", player: "p1" });
    return true;
  }
...
function updateRescue(s: PvpPlayerState, dt: number, canvasWidth: number, canvasHeight: number, effects: SimulationSideEffect[]) {
...
function updateTubeRescue(s: PvpPlayerState, dt: number, canvasWidth: number, canvasHeight: number, effects: SimulationSideEffect[]) {
...
export function updatePlayerState(
  ...
  // Handle rescues
  if (s.rescue.active) {
    updateRescue(s, dt, canvasWidth, canvasHeight, effects);
    return false;
  }
  if (s.tubeRescue.active) {
    updateTubeRescue(s, dt, canvasWidth, canvasHeight, effects);
    return false;
  }
  ...
  if (s.oxygen <= 0) {
    ...
    s.deathCause = "oxygen";
    s.alive = false;
    effects.push({ type: "PLAYER_DIED", player: "p1", cause: "oxygen" });
    return true;
  }
  ...
  // Jump buffer
  if (s.jumpBufferTimer > 0) {
    if (attemptJump(s, false, powerUpMode, effects)) {
      s.jumpBufferTimer = 0;
    }
  }
  ...
  if (player.y > canvasHeight) {
    ...
    effects.push({ type: "PLAY_SOUND", sound: player.isTrapped ? "die_quicksand" : "die_fall", player: "p1" });
    effects.push({ type: "PLAYER_DIED", player: "p1", cause: s.deathCause });
    s.alive = false;
    return true;
  }
    ...
    if (
      ...
      if (powerUpMode === "score_attack") {
        if (item.type === "OXYGEN") {
          ...
          effects.push({ type: "PLAY_SOUND", sound: "oxygen", player: "p1" });
          return false;
        }
        if (item.type === "SWORDFISH") {
          s.scoreAttackBonus += 300;
          addScorePopup(s, "+300", item.x, item.y);
          effects.push({ type: "PLAY_SOUND", sound: "swordfish", player: "p1" });
          return false;
        }
        ...
        if (item.type === "TURTLE_SHELL") {
          ...
          effects.push({ type: "PLAY_SOUND", sound: "shell_crack", player: "p1" });
          return false;
        }
        if (item.type === "URCHIN") {
          ...
          effects.push({ type: "PLAY_SOUND", sound: "die_urchin", player: "p1" });
          item.isDead = true;
          item.dy = -5;
          return true;
        }
      ...
      if (powerUpMode === "none") {
        ...
        if (item.type === "URCHIN") {
          ...
          s.deathCause = "urchin";
          s.alive = false;
          effects.push({ type: "PLAYER_DIED", player: "p1", cause: "urchin" });
          return false;
        }
        return true;
      }
