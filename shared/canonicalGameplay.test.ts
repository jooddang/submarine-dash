...
describe('canonical gameplay contract', () => {
  ...
  it('normalizes exact Road payloads and validates acknowledgements', async () => {
    expect(canonicalGameplayRequest({canonicalToken:'token',idempotencyKey:key,runEvidenceId:run,event})).toMatchObject({
      idempotencyKey:key,runEvidenceId:run,eventType:'run_end',contractVersion:'submarine-gameplay-v1',
    ...
    await expect(executeExpressCanonicalGameplay({canonicalToken:'token',idempotencyKey:key,runEvidenceId:run,event,
      roadcrosserRequest:road})).resolves.toBe(response);
    expect(road).toHaveBeenCalledWith('/api/internal/submarine-dash/mutations/settle-gameplay',expect.objectContaining({sessionToken:'token'}));
  });
});
